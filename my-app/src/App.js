...
// Custom color picker using <svg> in lieu of being any good at css lol
...
/*** React functions ***/
    ...
    getDefaultProps: function () {
        return {
            ...
            width: 300,
            height: 300,
            sliderWidthScale: 0.9,
            sliderHeightScale: 0.05,
            locked: false
        };
    },
...
/*** Misc/Utility/Whatevs functions ***/
     ...
     * h, s, v
    */
    HSBtoRGB: function HSBtoRGB(h, s, v) {
        var r, g, b, i, f, p, q, t;
    ...
    generateBackgroundColorStyleString: function generateBackgroundColorStyleString(hsbColor) {
        var rgbColor = this.HSBtoRGB(hsbColor[0], hsbColor[1], hsbColor[2]);
...
/*** Event handlers ***/
    ...
    onWheelMouseMove: function onWheelMouseMove(e) {
        ...
        if (this.state.draggingWheelHandle) {
            ...
            var wheelElement = document.getElementById("jamWheel-" + this.props.jamWheelId);
    ...
    onSliderMouseMove: function onSliderMouseMove(sliderHandleRadius, sliderId, adjustingValueIndex, e) {
        ...
        if (this.state.draggingSliderHandle && sliderId == this.state.draggingSliderId) {
            ...
            var cursorLocationX = e.clientX - sliderElement.getBoundingClientRect().x;

            // Get the new value for this slider clamped to 0-1
            var adjustedValue = Math.max(Math.min((cursorLocationX - sliderHandleRadius) / (sliderElement.getAttribute("width") - (2 * sliderHandleRadius)), 1), 0);
...
/*** Render function ***/
	render: function render() {
		...
		for (var i = 0; i < me.state.colorList.length; i++) {
		    var rgbFill = me.generateBackgroundColorStyleString([me.state.colorList[i][0], me.state.colorList[i][1], me.state.colorList[i][2]]);
		    ...
		    if (i == me.state.selectedHandleIndex) {
		        ...
		        selectedColorRGBFillMinSaturation = me.generateBackgroundColorStyleString([me.state.colorList[i][0], 0, 1]);
		        selectedColorRGBFillMaxSaturation = me.generateBackgroundColorStyleString([me.state.colorList[i][0], 1, 1]);
		        ...
		        if (me.state.colorList[i][2] <= 0) {
		            ...
		            selectedColorRGBFillMinBrightness = me.generateBackgroundColorStyleString([me.state.colorList[i][0], me.state.colorList[i][1], 0]);
		        } 
		        ...
		        if (me.state.colorList[i][2] >= 100) {
		            ...
		            selectedColorRGBFillMaxBrightness = me.generateBackgroundColorStyleString([me.state.colorList[i][0], me.state.colorList[i][1], 1]);
		        }
		    }
		...
		var hueSliderHandleRGBFill = me.generateBackgroundColorStyleString([selectedColor[0], 1, 1]);
		// The saturation slider handle only considers the hue/saturation of the selected color
		var saturationSliderHandleRGBFill = me.generateBackgroundColorStyleString([selectedColor[0], selectedColor[1], 1]);
...
var JamWheelSlider = createReactClass({
/*** Event handlers ***/
    ...
    render: function render() {
        ...
        if (!me.props.locked) {
            var globalToggleButtonStyle = {
                position: "absolute",
                left: (sliderWidth * 1.3) + (me.props.width / 3),
